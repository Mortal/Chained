...
void Root::Key(int key, byte dir) {
    ...
    switch (state) {
        ...
        break;
        case STATE_GAME:
            if DEBUGIT_KEYS logfile << "Root::key      " << ((char) (key & 0xFF)) << " direction " << ((int) (dir)) << std::endl;
            if (dir == KEYTYPE_DOWN) {
                switch (key) {
                    case VK_F1:
                        newnumply = 1;
        ...
        case STATE_DEBUG:
            ...
            logfile << "Root::key      " << stuf << std::endl;
            delete stuf;
            stuf = 0;
        break;
    }
    ...
    switch (state) {
        ...
        case STATE_DEBUG:
            for (int i = 1; i <= 8; i++) {
                ...
                if DEBUGIT_ROOTDRAW logfile << "Root::draw      " << x << "," << y << " ";
                ...
                glVertex2f (x, y); // SE
                y = oldy;
                if DEBUGIT_ROOTDRAW logfile << "Root::draw      " << x << "," << y << std::endl;
    ...
    for (int i = 1; i <= 9; ++i) {
        ...
        for (int j = 1; j <= 3; ++j) {
            ...
            for (int k = 0; k < BLOCK_MAXFRAMES; ++k) {
                ...
                FILE* intex = fopen(filename, "rb");
                
                fread(image, size, 1, intex);
